 ...
 */
const formatUser = (title) => {// Buat function formatUser
    return new Promise((resolve,reject)=>{
        const data = users.map((user)=>{ //Format nama user.
            return{
        ...
        setTimeout(()=>{
            resolve(data);
        },3000); //Fungsi membutuhkan waktu 3 detik.
    });
};
...
const findByName = (name) => {
    return new Promise((resolve,reject)=>{
        const data = users.find((user)=>{ //Find by nama user.
            return user.name == name;
        });
        setTimeout(()=>{
            resolve(data);
        },2000); //Fungsi membutuhkan waktu 2 detik.
    });
};
...
const filterByMajor = (major) => {
    return new Promise((resolve,reject)=>{
        const data = users.filter((user)=>{ //Find all user by major.
            return user.major == major;
        });
        setTimeout(()=>{
            resolve(data);
        },4000); //Fungsi membutuhkan waktu 4 detik.
    });
};
